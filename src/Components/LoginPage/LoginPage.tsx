...
const LoginPage = () => {
  ...
  React.useEffect(() => {
    ...
    if((nowPage.toLowerCase() !== "type=login" && nextPage !== "Sign In") || (nowPage.toLowerCase() === "type=login" && nextPage === "Sign In")) {
      dispatch(changeLoginText(""));
      dispatch(changePasswordConfirmValue(""));
      dispatch(changePasswordText(""));
    }
    changePage(
        nowPage.toLowerCase() !== "type=login" ? "Sign In" : "Registration"
    );
    document.title =
        nowPage.toLowerCase() === "type=login"
            ? "App | Sign In"
            : "App | Registration";
  }, [location]);
    ...
    <div className={styles.loginPage}>
      ...
      >
        ...
        <p className={styles.text}>
          if you {nextPage.toLowerCase() !== "sign in" ? `don't an` : "have"}{" "}
          account you can{" "}
          <Link
            to={
              nextPage.toLowerCase() === "sign in"
                ? "/sign?type=login"
